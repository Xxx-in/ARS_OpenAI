...
def fitness_function(q_table):
    pole_angle = [-1, -0.2, 0,	0.1, 0.2, 0.4]  #array to store observation values that corresponding to each bucket of pole velocity (after state discretization)
    pole_tip_velocity = [-1, -0.4, 0.6]         #array to store observation values that corresponding to each bucket of  angular velocity at pole tip (after state discretization)
    rewards_record = np.zeros([NUM_BUCKETS[2] , NUM_BUCKETS[3]])    #array with size of discretized state space to hold total reward obtained when starting from each state
    ...
    for x in range(NUM_BUCKETS[2]):
        for y in range(NUM_BUCKETS[3]):
            ...
            while(not(done)):
                ...
                # Execute the action
                obv, reward, done, _ = env2.step(action_0)
...
def mean_merge_CO (parent1_q_table, parent2_q_table):
        ...
        for y in range(NUM_BUCKETS[3]):
            for z in range(NUM_ACTIONS): 
                if (random.random() <= 0.95):    # determine if crossover happens 
                    child1_q_table[0][0][x][y][z] = 0.5 * parent1_q_table[0][0][x][y][z] + 0.5 * parent2_q_table[0][0][x][y][z]
                    child2_q_table[0][0][x][y][z] = 0.5 * parent1_q_table[0][0][x][y][z] + 0.5 * parent2_q_table[0][0][x][y][z]
...
if __name__ == "__main__":
    ...
    q = open("geneAlgo_meanWeightCO_ff1_qtable.txt", "w")
    r = open("geneAlgo_meanWeightCO_ff1_reward.txt", "w")
    e = open("geneAlgo_meanWeightCO_ff1_episode.txt", "a")
    ff = open("geneAlgo_meanWeightCO_ff1_fitness.txt", "a")
    ...
    child1_q_table,child2_q_table = mean_merge_CO(parent1_q_table, parent2_q_table) # crossover to form another 2 solution
    
    #compute fitness
    parent1_fitness= fitness_function(parent1_q_table)  
    parent2_fitness = fitness_function(parent2_q_table) 
    child1_fitness = fitness_function(child1_q_table)
    child2_fitness  = fitness_function(child2_q_table)
    ...
    # for i in range (1000):
        ...
        #crossover
        child1_q_table, child2_q_table = mean_merge_CO(parent1_q_table, parent2_q_table)
